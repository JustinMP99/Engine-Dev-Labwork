...
namespace end
{
	...
	public:
		...
		{
			//int16_t last_index = active_count - 1;
			////copy particle at index
			//T Temp = pool[index];
			////set the element at index equal to the value at active count
			//pool[index] = pool[last_index];
			////Set the value at active count to nullptr
			//pool[last_index] = Temp;
